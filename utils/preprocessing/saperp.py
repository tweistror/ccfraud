...
class Preprocess_saperp:
    def __init__(self, dataset_string, path, fraud_only):
    ...
    def initial_processing(self):
        ...
        if self.dataset_string == "saperp-ek":
            x_ben, x_fraud = self.initial_loading_ek()
        else:
            x_ben, x_fraud = self.initial_loading_vk()
    ...
    def load_data_EK_gen(self):
        ...
        df_EK_invoice = pd.read_csv(filepath_or_buffer=f'{path}/generated_purchasing_invoices.csv', sep=';',
                                    encoding='utf-8',
                                    usecols=['Referenz', 'ReferenzPosition', 'RechnungsBelegnummer',
                                             'RechnungsPosition'])

        df_EK_payments = pd.read_csv(filepath_or_buffer=f'{path}/generated_payments.csv', sep=';',
                                     encoding='utf-8',
                                     usecols=['Referenz', 'ReferenzPosition', 'Kontonummer', 'Soll/Haben-Kennz.',
                                              'Label'])
        df_EK_payments = df_EK_payments[df_EK_payments['Soll/Haben-Kennz.'] == 'S']

        df_EK = df_EK.merge(df_EK_invoice, left_on=['Einkaufsbeleg', 'Position'],
        ...
        df_EK = df_EK[
            ['Lieferant', 'Material', 'Werk', 'Angelegt von', 'Kontonummer', 'Bestellmenge', 'Bestellnettopreis',
             'Angelegt am', 'Uhrzeit', 'Label']]

        return df_EK

    def prepare_data_EK_gen(self, df_EK, use_numeric=True, log_scale=False):
    ...
    def initial_loading_vk(self):
        df = self.load_data_VK_gen()
        df = self.prepare_data_VK_gen(df)
    ...
    def prepare_data_VK_gen(self, df_VK, use_numeric=True, log_scale=False):
