...
from sklearn.metrics import precision_recall_curve, roc_auc_score, confusion_matrix, precision_recall_fscore_support, \
    accuracy_score
...
class DenoisingAutoencoder:
    ...
    def set_parameters(self, parameters):
        ...
        self.noise_factor = parameters['noise_factor']
    ...
    def build(self):
        ...
        x_train_noisy = x_train_split + noise_factor * np.random.normal(size=x_train_split.shape)
        x_train_noisy = np.clip(x_train_noisy, 0.0, 1.0)

        np.random.seed(self.seed)
        x_valid_noisy = x_valid_split + noise_factor * np.random.normal(size=x_valid_split.shape)
        x_valid_noisy = np.clip(x_valid_noisy, 0.0, 1.0)

        autoencoder.fit(x_train_split, x_train_noisy,
                        epochs=self.epochs,
                        batch_size=self.batch_size,
                        validation_data=(x_valid_split, x_valid_noisy),
    ...
    def predict(self, x_test, y_test):
        ...
        y_pred = self.autoencoder.predict(x_test_noisy)
    ...
    def plot_reconstructed_images(self, x_test, image_creator):
        ...
        x_test_noisy = x_test + noise_factor * np.random.normal(size=x_test.shape)
        x_test_noisy = np.clip(x_test_noisy, 0.0, 1.0)
        reconstructed_x_test = self.autoencoder.predict(x_test_noisy)
